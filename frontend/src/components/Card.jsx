...
export function Card({ id, data, title, description, imageUrl, owner, voteCount, onDelete, onVote, onClick, modalData }) {
    ...
    return (
        <div>
            <div id="card" onClick={() => { onClick(); modalData(data);}}>
                ...
                <div className="card-info">
                    ...
                    <p>{description}</p>
                    <p>{owner}</p>
                    <button onClick={(e) => { e.stopPropagation(); e.preventDefault(); handleUpvote(); }}>{`Upvote: ${voteCount}`}</button>
                </div>
            </div>
        </div>
    )
